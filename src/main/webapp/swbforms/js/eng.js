...
//        if(arr)ret=arr;
    ...
    {
        if (eng.submitTimeOut)
            clearTimeout(eng.submitTimeOut);
...
//                }
    ...
    validateForm: function (form)
    {
        ...
        if (form.linkToForms)
        {
            ...
            {
                var to = form.linkToForms[i];
                if (eng.validateForm(to.form) == false)
                    return false;
            }
        }
        ...
        if (form.formGrids)
        {
            ...
            {
                var grid = form.formGrids[i];
                if (grid.canvasItem.validate() == false)
                    return false;
            }
        }
        return true;
            ...
            } else
            {
                var inter = window.setInterval(function ()
                {
                    ...
                    for (var i = eng.validates.length; i--; )
                    {
                        ...
                        if (frm.isPendingAsyncValidation() == true)
                            end = false;
                    }
                    if (end == true)
                    {
...
//        }
    ...
    getDataSourceObjDef: function (dsDef, clone)
    {
        ...
        if (dsDef)
        {
            ...
            {
                if (dsDef.dsName)
                    dsObjDef.dsName = dsDef.dsName;
                if (dsDef.dsId)
                    dsObjDef.dsId = dsDef.dsId;
                else
                {
...
*/
    ...
    processFields: function (fields)
    {
        if (fields)
        {
            for (var x = 0; x < fields.length; x++)
            {
                ...
                if (fields[x].fields)
                    eng.processFields(fields[x].fields);
            }
    ...
    utils: {
         ...
         * @returns {void}
         */
        getASynchData: function (url, data, method, callback)
        {
            if (typeof XMLHttpRequest === "undefined")
            {
                XMLHttpRequest = function () {
    ...
    getDataSource: function (dsname)
    {
        return {
            ...
            invokeDS: function (data, callback)
            {
                eng.utils.getASynchData(eng.dataSourceServlet + "?dssp=" + eng.dataSourceScriptPath + "&ds=" + this.name, JSON.stringify(data), null, function (res) {
                    if (res.status == 200)
                    {
                        ...
                        if (ret.response)
                        {
                            ...
                            eng.showError("Bad DSResponse (" + res.response + ")");
                    } else
                        eng.showError("Bad HttpResponse (" + res.status + ")");
                });
            ...
            fetch: function (query, callback)
            {
                ...
                data.operationType = "fetch";
                 ...
                 res =this.invokeDS(data, function (res) {
                    callback(res);
                });

            },
            fetchObj: function (obj, callback)
            {
                var data = {};
            ...
            fetchObjById: function (id, callback)
            {
                var data = {data: {"_id": id}};

                this.fetch(data, function (res) {
            ...
            add: function (data, callback)
            {
                ...
                this.invokeDS(data, function (res) {
                    callback(res);
                });
            },
            addObj: function (obj,callback)
            {
                eng.utils.filterObj(obj);
                this.add({data: obj},function(ret){
                    if (ret && ret.status === 0)
                        obj._id = ret.data._id;
                    callback(ret);
                });
            ...
            updateObj: function (obj,callback)
            {
                eng.utils.filterObj(obj);
                return this.update({data: obj},callback);
            },
            ...
            validateObj: function (obj,callback)
            {
                eng.utils.filterObj(obj);
                return this.validate({data: obj},callback);
            },
            $asArray: function ()
            {
                var _this = this;
    ...
    login: function (username, password, callback)
    {
        ...
        eng.utils.getASynchData(eng.dataSourceServlet + "?dssp=" + eng.dataSourceScriptPath, JSON.stringify(data), null, function (res) {
            if (res.status == 200)
            {
